...
void gameDraw(GAME* g)
{
	...
	for (int i = 0; i < (BOARD_HEIGHT / BOARD_CELL); i += 2)
		rectDraw((BOARD_WIDTH - BOARD_CELL) >> 1, (i + 1) * BOARD_CELL, BOARD_CELL, BOARD_CELL, CR_GRAY);

	/* hiển thị điểm số của 2 người chơi */
	digitDraw((BOARD_WIDTH >> 1) - BOARD_CELL * 2 - DIGIT_WIDTH * BOARD_CELL, BOARD_TOP * 2, g->p1.score);
